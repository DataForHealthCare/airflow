...
previous_dt = current_datetime - timedelta(days=1)
previous_date = previous_dt.strftime("%Y-%m-%d")

# Claims driver start & end dates
claims_driver_start_date = "2024-01-03"
claims_driver_end_date = "2024-01-04"
...
with dag:
    admin_systems_ingestion_trigger_dates = DatabricksSubmitRunOperator(
        ...
        notebook_task={
            "notebook_path": "/Repos/f9656@healthpartners.com/Data-Ingestion-Pipeline/Databricks/ProductTeam/HICORE/claims_driver_monitor/orchestration_validation",
            "base_parameters": {
    ...
    def evaluate_admin_tables(ti=None):
        ...
        if result == "The ingestion dates for all AdminSystems tables are out-of-sync. The claims driver workflow will not be started.":
            ...
            Variable.set(key="monitor_data", value="", serialize_json=True)
            return False
        if result == "All AdminSystems tables have current ingestion dates. The claims driver workflow can be initiated.":
            Variable.set(key="adminsystem_stats", value={"adminsystem_validation": "AdminSystem tables are in-sync."}, serialize_json=True)
            return True         
    ...
    monitoring_table_workflow = DatabricksSubmitRunOperator(
        ...
        notebook_task={
            ...
            "base_parameters": {
                "source_environment": "prd",
                "target_environment": "dev",
                "monitoring_statistics": Variable.get("monitor_data"),
                "adminsystem_stats": Variable.get("adminsystem_stats"),
                "claims_driver_start_date": claims_driver_start_date,
                "claims_driver_end_date": claims_driver_end_date
            }
        },
